...
**状态：** 已更新 — 阶段二验证完成  
**日期：** 2026-04-30（基线：2026-04-28，验证：2026-04-29）  
**作者：** paddle-mx 团队  
...
## 8. 阶段二优化 — 各行动与实测结果（2026-04-29）

全部四个计划行动均于 2026-04-29 完成调查与测试。以下为结果汇总；
详细内容见 `task2-optimization/profiling/actions/`。

### 8.1 SOT 图预编译 — ❌ 放弃

**方案：** 启用 `graph_opt_level=1` 激活 SOT 静态操作树预编译，消除每步解码的 Python 重新调度开销。

**预期目标：** 10 → ≥13 tok/s（+30% 吞吐量）。

**实际结果：** 服务器在 MACA 3.3.0 上崩溃。  
设置 `graph_opt_level=1` 导致 FastDeploy Worker 进程在 SOT 图捕获阶段因 MACA 运行时错误而崩溃。MACA 3.3.0 SOT 后端与 PaddleOCR-VL 的动态控制流在当前平台/驱动版本下不兼容。

**决策：** ❌ **放弃。** 保持 `graph_opt_level=0`（默认值），不做任何配置修改。

---

### 8.2 MACA Shader 缓存（AOT 内核缓存） — ✅ 保留

**方案：** 利用 MACA 内置 Shader 缓存跨重启持久化已编译内核。同时在服务器就绪后执行启动预热脚本，提前触发图像专属内核形状编译。

**预期目标：** 图像冷启动 TTFT：135 秒 → <5 秒。

**实际结果（2026-04-29 测试）：**

| 指标 | 优化前 | 优化后 | 提升 |
|------|:------:|:-----:|:----:|
| 图像冷启动 TTFT（重启后首次图像请求） | **135.2 秒** | **4.28 秒** | **−97%** ✅ |
| 配合预热脚本的冷启动 TTFT | 135.2 秒 | **3.44 秒** | **−97.5%** ✅ |
| 预热后 TTFT（后续请求） | 4.38 秒 | 4.38 秒 | 无变化 |

机制：MACA 运行时自动将已编译 Shader 持久化到 `~/.metax/shadercache/<hash>_3.3.0.15.cache`（86 MB）。135.2 秒惩罚为**一次性部署成本**——后续重启使用缓存，首次图像 TTFT 降至约 4.28 秒。配合启动预热脚本可进一步降至 ~3.44 秒。

**决策：** ✅ **保留。** 无需修改服务器配置。推荐部署操作：将 `/root/.metax/shadercache/` 备份为部署物料，使新实例跳过 135 秒冷启动。

---

### 8.3 并发请求批处理 — ✅ 保留

**方案：** 向 FastDeploy 连续批处理调度器发送并发请求。服务器已配置 `--max-num-seqs 4`，无需修改。

**预期目标：** batch=4 聚合吞吐量 ~18–20 tok/s（+80–100%）。

**实际结果（2026-04-29 测试）：**

注：测试时 MACA Shader 缓存已预热，因此单请求基线已从 ~10 tok/s 提升至 ~45.9 tok/s（消除了此前被混入基线测量的 JIT 开销）。**批处理相对同等条件下顺序请求的增益**是关键指标：

| 批量大小 | 聚合吞吐量 | 相比 batch=1（同等条件） | 备注 |
|:--------:|:---------:|:------------------------:|------|
| 1（顺序） | **45.9 tok/s** | — | Shader 缓存预热后基线 |
| 2（并发） | **65.2 tok/s 均值** / 87 峰值 | **+42% 均值 / +90% 峰值** | 高方差（输出长度不定） |
| 4（并发） | **87.8 tok/s 均值** / 97 峰值 | **+91% 均值** | 稳定；接近 2× |

**关键观察：** batch=4 时单请求吞吐保持在 ~44 tok/s（无显著个体延迟损耗），聚合吞吐接近翻倍，原因是每步解码的 Python 调度开销在多个并发请求间被摊销。

**决策：** ✅ **保留。** `--max-num-seqs 4` 已配置，无需修改。+91% 聚合增益需要客户端同时发送并发请求。

---

### 8.4 RMSNorm + 线性层融合 — ⚠️ 受阻（未来工作）

**方案：** 将 5,903 次 RMSNorm 调用与后续线性投影内核融合，减少约 30% kernel launch 次数，消除冗余全局内存往返。

**预期目标：** +5–8% GPU 吞吐。

**实际结果（2026-04-29 调查）：**

FastDeploy 2.5 已使用 `paddle.incubate.nn.functional.fused_rms_norm` 调度至 `fused_rms_norm_ext_metax_gpu`——这是 Paddle 中最优化的单算子 Norm 实现。但更深层的 **RMSNorm + Linear** 融合内核在以下组件中均不存在：
- FastDeploy 2.5.0
- PaddlePaddle incubate 算子集
- MetaX 自定义设备插件（MACA 3.3.0，`libpaddle-metax-gpu.so`）

当前无任何配置参数、图优化 Pass 或环境变量可启用此融合。实现此融合需要编写自定义 MACA/CUDA 内核。

**决策：** ⚠️ **受阻 — 未来工作。** 记录为阶段三任务：面向 18 层解码路径的自定义 MACA 融合 `RMSNorm+GEMV` 内核。
...
| 含预热脚本的冷启动 TTFT | **135.2 秒** | **3.44 秒** | **−97.5%** ✅ |
| 预热后 TTFT（图像，628 token） | **4.38 秒** | **4.38 秒** | 无变化 |
| 聚合吞吐量（batch=4） | ~10 tok/s¹ | **~88 tok/s** | **+780%** ✅ |
| 单请求解码速度（batch=1） | ~10 tok/s¹ | **~46 tok/s** | **+360%** ✅ |

¹ 阶段一 ~10 tok/s 基线中混入了首次运行时的 JIT 开销。Shader 缓存预热后，真实单请求吞吐为 ~46 tok/s；+91% 批处理增益以同等条件为参照。

**≥20% 目标已大幅超越。** 主要增益来自 MACA Shader 缓存持久化（行动 8.2）和并发批处理（行动 8.3），两项优化均**无需修改服务器配置**。
...
| 图像冷启动 TTFT | 135.2 秒 | **4.28 秒** | **−97%** ✅ |
| 含预热脚本的冷启动 TTFT | 135.2 秒 | **3.44 秒** | **−97.5%** ✅ |
| 预热后 TTFT（图像，628 token） | 4.38 秒 | 4.38 秒 | — |
| 单请求解码速度（batch=1） | ~10 tok/s | **~46 tok/s** | **+360%** ✅ |
| 聚合吞吐量（batch=4） | ~10 tok/s | **~88 tok/s** | **+780%** ✅ |
| SOT 图预编译 | 计划中（+30%） | ❌ 放弃 | MACA 3.3.0 崩溃 |
| RMSNorm+线性层融合 | 计划中（+5–8%） | ⚠️ 受阻 | 无可用内核 |

### 9.3 行动决策汇总

| 行动 | 描述 | 决策 | 原因 |
|------|------|:---:|------|
| 8.1 | SOT 图预编译 | ❌ 放弃 | MACA 3.3.0 服务器崩溃 |
| 8.2 | MACA Shader 缓存 | ✅ 保留 | −97% 冷启动，自动生效 |
| 8.3 | 并发批处理 | ✅ 保留 | +91% 聚合吞吐 |
| 8.4 | RMSNorm+线性层融合 | ⚠️ 未来工作 | 当前栈中无可用内核 |
